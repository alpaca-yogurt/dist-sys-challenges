                ...
                if 0 < len(entries) or self.heartbeat_interval < elapsed_time:
                    ...
                    def handler(res):
                        ...
                        if self.state == "leader" and term == self.current_term:
                            self.reset_step_down_deadline()
                            if body["success"]:
                                self.next_index[_node] = max(self.next_index[_node], _ni + len(_entries))
                                self._match_index[_node] = max(self._match_index[_node], _ni - 1 + len(_entries))
    ...
    def setup_handlers(self):
        ...
        def append_entries(msg):
            ...
            if body["prev_log_index"] <= 0:
                raise RuntimeError("Out of bounds previous log index" + str(body["prev_log_index"]))
        ...
        def kv_req(msg):
            if self.state == "leader":
                ...
                self.net.reply(msg, {"type": "error", "code": 11, "text": "not a leader"})
